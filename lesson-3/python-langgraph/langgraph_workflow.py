...
def parallelization():
    class ParallelState(TypedDict):
        topic: str
        joke: str; story: str; poem: str
        combined_output: str

    def joke_node(state): return {"joke": llm.invoke(f"写一个笑话，主题词为： {state['topic']}").content}
    def story_node(state): return {"story": llm.invoke(f"写一个故事，主题词为： {state['topic']}").content}
    def poem_node(state): return {"poem": llm.invoke(f"写一首诗歌，主题词为： {state['topic']}").content}
    ...
    parallel_workflow = (
        ...
        .add_node("joke", joke_node).add_node("story", story_node).add_node("poem", poem_node)
        .add_node("aggregator", aggregator)
        # START 同时连接到三个节点，实现并行
        .add_edge(START, "joke").add_edge(START, "story").add_edge(START, "poem")
        .add_edge("joke", "aggregator").add_edge("story", "aggregator").add_edge("poem", "aggregator")
...
def route_workflow():
    ...
    def classify_intent(state: IntentState):
        ...
        intent = router.invoke([
            SystemMessage(content="你是一个意图识别助手，用户可能会问天气、翻译或闲聊。"),
    ...
    def router_fn(state):
        return state["intent"]
    def handle_weather(state): return {"result": f"天气查询: {state['query']} -> 晴 25度"}
    def handle_translate(state): return {"result": f"翻译: {state['query']} -> Hello World"}
    def handle_chat(state): return {"result": f"闲聊: {state['query']} -> 你好呀！"}
...
def orchestrator_worker_workflow():
    ...
    def orchestrator(state: ReportState):
        ...
        sections = planner.invoke([
            SystemMessage(content="你是一个资深报告撰写专家，负责根据主题生成报告章节大纲。"),
            HumanMessage(content=f"请根据主题 '{state['topic']}' 生成报告的{state['sections_num']}个章节大纲，返回章节名称和内容概述。")
        ])
    ...
    def abstract_worker(state: WorkerState):
        ...
        section_content = llm.invoke([
            SystemMessage(content="你是一个资深报告撰写专家，负责根据章节大纲生成章节内容。"),
            HumanMessage(content=f"请根据章节名称 '{state['section'].name}' 和内容概述 '{state['section'].description}' 生成完整的章节内容。")
        ])
    ...
    # 执行
    state = orchestrator_worker_graph.invoke({"topic": "创建一份关于 LLM scaling laws 的报告", "sections_num": 3, "sections": [], "completed_sections": [], "final_report": ""})
