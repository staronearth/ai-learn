...
def make_search_knowledge_rag(recorder):
    ...
    def search_knowledge_rag(query, top_k=5):
        ...
        ds = splitter_markdown()
        #1.稠密检索
        dense_vector = retrieve_dense(ds)
        dense_results = dense_vector.similarity_search(query, top_k)
        dense_vr = [
            {"id": doc.metadata.get("doc_id", doc.id), "content": doc.page_content} for doc in dense_results
        ]

        #2.稀疏检索
        bm25_retrieve = retrieve_bm25(ds, top_k)
        query_tokens = [list(jieba.cut(query))]
        bm25_retrieve_results, bm25_retrieve_scores = bm25_retrieve.retrieve(query_tokens, k=top_k)
        bm25_tuple_reslut = [(bm25_retrieve_results[0, i], bm25_retrieve_scores[0, i]) for i in range(bm25_retrieve_results.shape[1])]
